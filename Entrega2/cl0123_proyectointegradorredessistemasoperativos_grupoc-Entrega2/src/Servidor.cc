...
{
	string cod404 = "Error! Recurso no encontrado,";
	string cod400 = "Error! Solicitud inválida,";
	char *respFinal;
	switch (codigo)
	{
	case 400:
	...
	{ // consola
		...
		if (solicitudFinal != "400")
		{
			if (slashesCuenta == 1)
			{
				...
				if (estaPais)
				{
					char *respuestaFinal = solicitud.getPais(solicitudFinal);
					write(s3->getIDSocket(), respuestaFinal, strlen(respuestaFinal));
				}
				else
				{
			...
			else if (slashesCuenta == 2)
			{
				...
				{
					string cantonTilde = solicitud.verificarNombreCanton(solicitudFinal);
					char *respuestaFinal = solicitud.getCanton(cantonTilde);
					write(s3->getIDSocket(), respuestaFinal, strlen(respuestaFinal));
				}
				else
				{
